module Rails3JQueryAutocomplete
  module Autocomplete
    ...
    #
    def json_for_autocomplete(items, value_method, label_method, extra_data=[])
      items.collect do |item|
        hash = {"id" => item.id.to_s, "label" => item.send(label_method), "value" => item.send(value_method)}
